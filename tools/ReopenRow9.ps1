#ReopenRow9.ps1 - Reopen row 9 (khoqua09), verify active_result + state
...
Write-Host "row9 chk=$(Row-Chk $snap 9) epoch=$($snap.sepoch)" -ForegroundColor Cyan
$ws.Dispose()

Write-Host "### reopen row 9 ###" -ForegroundColor Magenta
$ws = New-WS; $snap = Get-Snapshot $ws
$id = Make-Id
$msg = '{"t":"act","key":"root/1000#0","op":"row_toggle","r":9,"id":"' + $id + '","epoch":' + $snap.sepoch + '}'
...
Write-Host "### after (wait 8s) ###" -ForegroundColor Magenta
Start-Sleep -Seconds 8
$ws = New-WS; $snap = Get-Snapshot $ws
Write-Host "row9 chk=$(Row-Chk $snap 9)  $(if ((Row-Chk $snap 9) -eq 1) {'[RUNNING OK]'} elseif ((Row-Chk $snap 9) -eq 0) {'[still closed]'} else {'[unknown]'})" -ForegroundColor Cyan
$ws.Dispose()